...
async def mybot():
    ...
    if bot_name.startswith("مسـاعـد"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "Power_Thon")
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت باورثـون المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name} ")
            await asyncio.sleep(3)
...
zthon = {"@Power_Thon", "@group_powerr", "@VipDDe", "@tttfftttt"}
async def saves():
   for Cat in zthon:
        try:
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كـروب السجـل باورثـون", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
